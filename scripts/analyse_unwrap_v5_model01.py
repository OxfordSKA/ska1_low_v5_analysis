...
def main():
    ...
    name = 'model01'  # Name of the telescope model (prefix in file names)
    # -------------------------------------------------------------------------
    ...
    # TODO(BM) move these into the telescope class v5 method?
    b = 0.515
    theta0_deg = -48
    start_inner = 417.82
    num_arms = 3
    ...
    # TODO(BM) separate telescope and analysis class to
    # be able to pass a series of telescopes to the analysis
    # and build up a dict of metrics with the telescope name as the key.
    metrics = AnalyseMetrics(out_dir)

    # Loop over telescope models
    # - in each model we replace an additional ring of clusters
    for i in range(cluster_r.size):
        ...
        # TODO(BM) move some of these loops into the telescope class
        for j in range(i + 1):
            ...
            for k in range(num_arms):
                ...
                tel.add_log_spiral_section(
                    stations_per_cluster, start_inner, cluster_x[idx],
                    cluster_y[idx], b, delta_theta_deg,
